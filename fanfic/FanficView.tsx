...
import { ArrowLeft, Search, User, Users, BookOpen, Send, Sparkles, Plus, Trash2, LayoutGrid, PenTool, ChevronDown, ChevronUp, X, Home, Download, Upload, RotateCcw } from 'lucide-react';
...
export const FanficView: React.FC<Props> = ({ onBack, onExit, onStartGame, settings }) => {
  ...
  useEffect(() => {
    const savedWorks = localStorage.getItem('custom_fanfic_works');
    if (savedWorks) {
      try {
  ...
  useEffect(() => {
    if (works !== FANFIC_WORKS) {
      localStorage.setItem('custom_fanfic_works', JSON.stringify(works));
    }
  }, [works]);
      ...
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          if (isValid) {
            ...
            alert('Nhập dữ liệu thành công! Các tác phẩm mới đã được gộp vào danh sách.');
          } else {
            alert('Tệp không đúng định dạng tác phẩm Đồng Nhân.');
          }
        }
      } catch (err) {
        alert('Lỗi khi đọc tệp: ' + err);
      }
    };
  ...
  const handleReset = () => {
    if (window.confirm('Bạn có chắc chắn muốn khôi phục danh sách tác phẩm gốc? Toàn bộ thay đổi đã nhập sẽ bị mất.')) {
      ...
      localStorage.removeItem('custom_fanfic_works');
      setSelectedWork(null);
