...
import { Trash2, Plus, Upload, ShieldCheck, Cpu, Zap, RefreshCw, Image as ImageIcon, X, AlertTriangle } from 'lucide-react';
      ...
      if (response.ok) {
        ...
        if (data && Array.isArray(data.data)) {
          ...
          if (models.length > 0 && !settings.proxyModel) {
            onUpdateSettings({ proxyModel: models[0], proxyStatus: 'success' });
          } else {
            onUpdateSettings({ proxyStatus: 'success' });
          }
                    ...
                    {settings.autoGenerateImages && (
                      <div className="space-y-4 pt-4 border-t border-white/5 animate-in fade-in slide-in-from-top-2 duration-300">
                        <div className="space-y-2">
                          <label className="text-[10px] font-black uppercase tracking-widest text-neutral-500">Mô hình tạo ảnh (Model)</label>
                          <div className="space-y-2">
                            <div className="relative">
                              <select
                                value={settings.imageModel || 'gemini-2.5-flash-image'}
                                onChange={(e) => onUpdateSettings({ imageModel: e.target.value })}
                              ...
                              >
                                <optgroup label="Mô hình mặc định (Gemini)" className="bg-[#080808] text-neutral-500">
                                  {IMAGE_MODELS_GEMINI.map(m => (
                                    <option key={m.id} value={m.id} className="bg-[#080808] text-white">{m.label}</option>
                                  ))}
                                </optgroup>
                              </select>
                            ...
                            <p className="text-[8px] text-neutral-600 font-bold uppercase italic">
                              * Các model Nano Banana được cung cấp miễn phí bởi Google AI Studio thông qua Gemini API Key của bạn.
                            </p>
                          </div>
                        </div>
                          ...
                          <div className="relative">
                            ...
                            >
                              {IMAGE_STYLES.map(style => (
                                <option key={style} value={style} className="bg-[#080808] text-white">{style}</option>
                              ))}
                            </select>
                            <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-cyan-500/50">
                              ▼
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between items-center">
                          <label className="mono text-[9px] font-black text-neutral-500 uppercase tracking-widest">1. URL Proxy (OpenAI Compatible)</label>
                          <button 
                      ...
                      {(availableModels.length > 0 || settings.proxyModel) && (
                        <div className="space-y-1.5 animate-in fade-in slide-in-from-top-2 duration-300">
                          <label className="mono text-[9px] font-black text-neutral-500 uppercase tracking-widest">1.1. Chọn Model Proxy</label>
                          <div className="relative">
                            <select 
                      ...
                      {/* Proxy Key */}
                      <div className="space-y-1.5">
                        <label className="mono text-[9px] font-black text-neutral-500 uppercase tracking-widest">2. Password / API Key</label>
                        <input 
                          type="password"
